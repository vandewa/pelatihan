...
<!-- ================================
  ...
  ================================= -->
<section
    class="footer-area section-bg-2 padding-top-100px padding-bottom-40px {{ request()->is('student/*') ? 'student-dashboard' : '' }}">
    <div class="container">
        <div class="row">
            <div class="{{ request()->is('student/*') ? 'col-lg-3 offset-md-2' : 'col-lg-4' }} column-td-half">
                <div class="footer-widget">
                    ...
                    <ul class="list-items footer-address">
                        <li>
                            <a
                                href="tel:{{getSystemSetting('type_number')->value}}">{{getSystemSetting('type_number')->value}}</a>
                        </li>
                    ...
                    <h3 class="widget-title font-size-17 mt-4">@translate(Kami ada di )</h3>
            ...
            <div class="{{ request()->is('student/*') ? 'col-lg-3' : 'col-lg-4' }} column-td-half">
                <div class="footer-widget">
                    <h3 class="widget-title">@translate(Perusahaan)</h3>
            ...
            <div class="{{ request()->is('student/*') ? 'col-lg-3' : 'col-lg-4' }} column-td-half">
                <div class="footer-widget">
                    <h3 class="widget-title">@translate(Pelatihan)</h3>
                    <span class="section-divider"></span>
                    <ul class="list-items">
                ...
                <div class="col-lg-2">
                    <div class="sort-ordering">
                        <form id="ru-currency" method="post" action="{{route('frontend.currencies.change')}}">
                            @csrf
                            <select class="sort-ordering-select selectpicker" data-live-search="true" tabindex="-98"
                                name="id" onchange="currencyChange()">
                                @foreach(\App\Model\Currency::where('is_published',true)->get() as $item)
                                <option value="{{$item->id}}" {{defaultCurrency()==$item->code ? 'selected' : null}}>
                                    {{Str::ucfirst($item->symbol.' '.$item->code)}}</option>
                                @endforeach
                            </select>
                        </form>
                    </div>
                ...
                <div class="col-lg-2">
                    <div class="sort-ordering my-2">
                        <form id="ru-lang" method="post" action="{{route('frontend.languages.change')}}">
                            @csrf
                            <select class="sort-ordering-select  selectpicker" tabindex="-98" name="code"
                                data-live-search="true" onchange="languageChange()">
                                @foreach(\App\Model\Language::all() as $language)
                                <option value="{{$language->code}}"
                                    {{(\Illuminate\Support\Facades\Session::get('locale')==$language->code ? 'selected'
                                    : env('DEFAULT_LANGUAGE') == $language->code ) ? 'selected' : null
