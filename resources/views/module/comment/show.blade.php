...
<div class="card m-2">
    <div class="card-header">
        <div class="float-left">
            <h2 class="card-title">@translate(Jawab Komentar Pelatihan)
                <a target="_blank" href="{{ route('course.show',[$comment->course->id,$comment->course->slug])}}">
                    @translate(Pelatihannya): <p>{{$comment->course->title ?? 'N/A'}}</p>
                </a>
            </h2>
        </div>
    </div>
                ...
                <div class="chat-body">
                    ...
                    <div
                        ...
                        <a target="_blank" class="{{route('messages.show', $item->user_id)}}">
                            <div>
                                <img class="align-self-center mr-3 text-center rounded-circle"
                                    src="{{filePath($item->user->image) ?? 'assets/images/users/girl.svg'}}" height="60"
                                    width="60">
                                <p>{{$item->user->name}}</p>
                            </div>
                        </a>
                        ...
                        <form action="{{route('comments.replay')}}" method="post">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="comment" required
                                    ...
                                    placeholder="@translate(Ketikka pesan...)" aria-label="Text"
                                    value="{{old('comment')}}">
